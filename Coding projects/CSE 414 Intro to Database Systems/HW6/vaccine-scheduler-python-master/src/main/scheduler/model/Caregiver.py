...
class Caregiver:
    ...
    # helper method
    def upload_availability_with_name(caregiver, d):
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()
        add_availability = "INSERT INTO Availabilities VALUES (%s , %s)"
    ...
    # helper method to remove an availability once it is reserved
    # caregiver is a string for the caregiver
    # date is a string for the date
    def delete_availability(caregiver, date):
            # maybe add checks here
            cm = ConnectionManager()
            conn = cm.create_connection()
            cursor = conn.cursor()
            delete_availability = "DELETE FROM Availabilities WHERE Caregiver = %s AND Time = %s"
            try:
                cursor.execute(delete_availability, (caregiver, date))
                conn.commit()
            except pymssql.Error:
    ...
    def is_available_times(date):
            cm = ConnectionManager()
            conn = cm.create_connection()
            select_avail = "SELECT * FROM Availabilities WHERE Time = %s;"
            try:
                cursor = conn.cursor(as_dict=True)
                cursor.execute(select_avail, date)
                for row in cursor:
                    if row:
                        return True
